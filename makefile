...
data\hrtime\migrate-card:
	python "$(MIGRATION_DIRECTORY)\migrate_sqlite.py" migrate -d $(DB_PLATFORM_PATH) -f "$(MIGRATION_DIRECTORY)\db_card.json"

data\hrtime\migrate-registration:
	python "$(MIGRATION_DIRECTORY)\migrate_sqlite.py" migrate -d $(DB_PLATFORM_PATH) -f "$(MIGRATION_DIRECTORY)\db_registration.json"

data\hrtime\migrate-worker:
	python "$(MIGRATION_DIRECTORY)\migrate_sqlite.py" migrate -d $(DB_PLATFORM_PATH) -f "$(MIGRATION_DIRECTORY)\db_user.json"
